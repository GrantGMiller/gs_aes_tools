...
class EthernetClientInterface:
    def __init__(self, *args, **kwargs):
        ...
        self._resetOnNextPing = False

        self.Hostname = self._interface.Hostname
        self.IPAddress = self._interface.IPAddress
        self.IPPort = self._interface.IPPort
        self.Protocol = self._interface.Protocol
        self.ServicePort = self._interface.ServicePort

        self._key = None
        self._userRxData = None

        self._interface.ReceiveData = self.__ReceiveData

    def __ReceiveData(self, interface, encrypted):
        decrypted = self._DoDecrypt(encrypted)
        print('__ReceiveData encrypted={}, decrypted={}'.format(encrypted, decrypted))
        if callable(self._userRxData):
            self._userRxData(self, decrypted)

    def SetEncryptionKey(self, key):
        print('SetEncryptionKey')
        self._key = key

    def Send(self, plain):
        encrypted = self._DoEncrypt(plain)
        print('Send plain={}, encrypted={}'.format(plain, encrypted))
        self._interface.Send(encrypted)

    def Connect(self, *args, **kwargs):
        return self._interface.Connect(*args, **kwargs)

    def Disconnect(self):
        self._interface.Disconnect()

    def StartKeepAlive(self, *args, **kwargs):
        return self._interface.StartKeepAlive(*args, **kwargs)

    def StopKeepAlive(self, *args, **kwargs):
        return self._interface.StopKeepAlive(*args, **kwargs)
    ...
    def Connected(self, func):
        self._interface.Connected = func

    @property
    def Disconnected(self):
        return self._interface.Disconnected

    @Disconnected.setter
    def Disconnected(self, func):
        self._interface.Disconnected = func

    @property
    def ReceiveData(self):
        return self._userRxData

    @ReceiveData.setter
    def ReceiveData(self, func):
        self._userRxData = func

    def _DoEncrypt(self, data):
        cipher = AES_Cipher(self._key)
        data = cipher.encrypt(data)
        return data

    def _DoDecrypt(self, data):
        return self._DoEncrypt(data)  # this AES is mirrored
...
class EthernetServerInterfaceEx:
    def __init__(self, *args, **kwargs):
        self._interface = extronlib.interface.EthernetServerInterfaceEx(*args, **kwargs)

        self._subClientObjects = []

        # duplicate
        self.Clients = self._subClientObjects
        self.IPPort = self._interface.IPPort
        self.Protocol = self._interface.Protocol
        self.Interface = self._interface.Interface
        self.MaxClients = self._interface.MaxClients

        self._keys = {
            # clientObject.IPAddress: key,
        }
        self._userRxData = None

        self._userConnectedHandler = None
        self._userDisconnectedHandler = None

        self._interface.ReceiveData = self.__ReceiveData

    def __ReceiveData(self, client, encrypted):
        # parse the encrypted data
        decrypted = self._DoDecrypt(client, encrypted)
        print('__ReceiveData encrypted={}, decrypted={}'.format(encrypted, decrypted))
        subClient = self._GetSubClientFromClient(client)

        # Do the callback
        if callable(self._userRxData):
            self._userRxData(subClient, decrypted)

    def SetEncryptionKey(self, IPAddress, key):
        print('SetEncryptionKey')
        self._keys[IPAddress] = key

    def Disconnect(self, client):
        self._interface.Disconnect(client)

    def StartListen(self, timeout=0):
        return self._interface.StartListen(timeout)

    def StopListen(self):
        return self._interface.StopListen()

    def _GetSubClientFromClient(self, client):
        for subClient in self._subClientObjects:
            if subClient._extronClientObj == client:
                return subClient

    def _GetClientByIPAddress(self, IPAddress):
    ...
    @Connected.setter
    def Connected(self, userConnectionHandler):
        self._userConnectedHandler = userConnectionHandler

        def AESServerConnectionHandler(client, state):
            if client not in self._subClientObjects:
                newSubClient = SubClientObject(self, client)

            if client.IPAddress not in self._keys:
                self._keys[client.IPAddress] = None

            userConnectionHandler(newSubClient, state)

        self._interface.Connected = AESServerConnectionHandler

    @property
    def Disconnected(self, userDisconnectedHandler):
        self._userDisconnectedHandler = userDisconnectedHandler

        def AESServerDisconnectionHandler(client, state):
            subClient = self._GetSubClientFromClient(client)

            self._subClientObjects.remove(subClient)
            self._keys.pop(client.IPAddress, None)

            userDisconnectedHandler(subClient, state)

        self._interface.Disconnected = AESServerDisconnectionHandler
    ...
    def _DoEncrypt(self, client, data):
        cipher = AES_Cipher(self._keys[client.IPAddress])
        data = cipher.encrypt(data)
        return data

    def _DoDecrypt(self, client, data):
        return self._DoEncrypt(client, data)  # this AES is mirrored


class SubClientObject:
    def __init__(self, parent, extronClientObj):
        self._parent = parent
        self._parent._subClientObjects.append(self)
        self._extronClientObj = extronClientObj

        self.Hostname = self._extronClientObj.Hostname
        self.IPAddress = self._extronClientObj.IPAddress
        self.ServicePort = self._extronClientObj.ServicePort

    def Send(self, plain):
        encrypted = self._parent._DoEncrypt(self._extronClientObj, plain)
        print('Send plain={}, encrypted={}'.format(plain, encrypted))
        self._extronClientObj.Send(encrypted)

    def Disconnect(self):
        return self._extronClientObj.Disconnect()
...
class File(extronlib.system.File):
    ...
    def write(self, data):
        print('aes_tools.File.write(data={})'.format(data))
        if self._key is not None:
            encryptedData = self._cipher.encrypt(data)
